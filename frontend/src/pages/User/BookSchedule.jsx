...
const BookSchedule = () => {
    ...
    const getData = () => {
        if (email != null) {
            axios.get(`http://localhost:3002/getSchedule/${email}`)
                .then((res) => {
    ...
    const userBooking = () => {
        if (date !== "" && time !== "") {
            let uid= uuidv4();
            axios.post(`http://localhost:3002/Booking/addBooking`, { name, email: UserEmail, doctorEmail: email, date, time,meetingId:uid,docName,expertise,sta:"Upcomming",reason:"N/A" })
                .then((res) => {
                    console.log(res.data)
                })
                .catch((e) => console.log(e))

            axios.patch('http://localhost:3002/scheduleStatus', { email, date, time })
                .then((res) => {
                    console.log(res.data)
                })
                .catch((e) => console.log(e))
